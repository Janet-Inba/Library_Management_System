...
<?php 
$rid=intval($_GET['rid']);
$sql = "SELECT tblstudents.StudentId ,tblstudents.FullName,tblstudents.EmailId,tblstudents.MobileNumber,tblbooks.BookName,tblbooks.ISBNNumber,tblissuedbookdetails.IssuesDate,tblissuedbookdetails.ReturnDate, tblissuedbookdetails.DueDate, tblissuedbookdetails.id as rid,tblissuedbookdetails.fine,tblissuedbookdetails.RetrunStatus,tblbooks.id as bid,tblbooks.bookImage from  tblissuedbookdetails join tblstudents on tblstudents.StudentId=tblissuedbookdetails.StudentId join tblbooks on tblbooks.id=tblissuedbookdetails.BookId where tblissuedbookdetails.id=:rid";
$query = $dbh -> prepare($sql);
$query->bindParam(':rid',$rid,PDO::PARAM_STR);
$query->execute();
...
if($result->fine=="")
{
	...
	$timeDiff = strtotime($currentDate) - strtotime($due_date);
	
	$secondsInADay = 60 * 60 * 24; // Number of seconds in a day
	$finePerDay = 2; // Fine amount per day
	$fine = 0;
	
	if ($timeDiff > 0) {
    	$daysLate = ceil($timeDiff / $secondsInADay);
    	$fine = $daysLate * $finePerDay;
		
		echo $daysLate . ' days overdue';
	}
	
  ?>
	
<input class="form-control" type="text" name="fine" id="fine" value="<?php echo $fine ?>" required />
